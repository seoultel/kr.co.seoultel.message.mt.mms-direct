...
public class KtfMMSReportUtil extends MMSReportUtil<KtfSoapMessage> {
    ...
    public void prepareToReport(MessageDelivery messageDelivery, KtfSoapMessage ktfSoapMessage) {
        ...
        boolean isSuccessToSend = ktfDeliveryReportReqMessage.getMmStatus().equals(KtfProtocol.KTF_REPORT_SUCCESS)
                                    || ktfDeliveryReportReqMessage.getMmStatus().equals(KtfProtocol.KTF_REPORT_SUCCESS_ANSIM);

        setDeliveryTypeAndStateAtReport(isSuccessToSend, messageDelivery); // set DeliveryType & DeliveryState
        FallbackUtil.setResult(messageDelivery, getReportResult(isSuccessToSend, messageDelivery, ktfDeliveryReportReqMessage));
    }
